...
public class AprilTagWebcam {
   ...
   public double getTargetTurretBearing(){
       this.update();
       ...
       //return estimateBearingFromPixels(target.center.x,640,70) - old code
       
       //trig eqautions
       double sideB = Math.sqrt(target.ftcPose.x*target.ftcPose.x+target.ftcPose.y*target.ftcPose.y);
       // side B is the distance from the camera to the AprilTag
       double sideA = 10.404 ;
       //side A is the distance from the camera to the center of the Turret
       double sideCX = target.ftcPose.x + 6;
       ...
       return -1*TurretBearing;
       //returns in the oposite direction


      
   }
   public double getTargetTurretDistance(){
    this.update();
       AprilTagDetection target = this.getTargetTag();
       if (target == null){
        return 0.0;
